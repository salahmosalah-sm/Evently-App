...
class RoutesManager {
  ...
  static Route? router(RouteSettings sitting) {
    switch (sitting.name) {
      ...
      case createEvent:
        final event = sitting.arguments as EventDM?;
        return MaterialPageRoute(builder: (_) => CreateEvent(event: event));
      ...
      case eventDetails:
        return MaterialPageRoute(
          builder:
              (context) => EventDetails(event: sitting.arguments as EventDM),
        );
      default:
        return MaterialPageRoute(builder: (context) => const SignIn());
    }
